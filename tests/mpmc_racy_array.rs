...
impl<T, const UNBOUNDED_BACKOFF: bool> Send2<T> for MTx<T, RacyArray<usize, UNBOUNDED_BACKOFF>> {
    fn send2(&self, msg: T, sync: bool) -> Result<(), SendError<T>> {
        if sync {
...
impl<T, const UNBOUNDED_BACKOFF: bool> Recv2<T> for MRx<T, RacyArray<usize, UNBOUNDED_BACKOFF>> {
    fn recv2(&self, sync: bool) -> Result<T, RecvError> {
        if sync {
            self.recv_blocking()
        } else {
...
fn mpmc<const UB: bool>(#[values(false, true)] sync: bool, #[values(FALSE, TRUE)] ub: Bool<UB>) {
    let _ = ub;
    let (tx, rx) = <RacyArray<_, UB>>::new(2).channel::<usize>();
...
fn try_send_full<const UB: bool>(#[values(FALSE, TRUE)] ub: Bool<UB>) {
    let _ = ub;
    let (tx, rx) = <RacyArray<_, UB>>::new(1).channel::<usize>();
    assert!(!tx.is_full());
...
fn drop_buffered(#[case] offset: usize, #[case] msgs: usize) {
    let arc = Arc::new(());
    let (tx, rx) = <RacyArray>::new(4).channel();
