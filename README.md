...
`kanal` is an MPMC channel which pretends to be faster than any other competitor. However, it uses a lock-based algorithm, which means every operation performs at least two atomic RMW operations, vs. one for `chenal`, without talking about other (spin-)lock drawbacks. In `kanal`'s own benchmark, `chenal` is 3x faster on average.
...
`crossfire` also overuses backoff loops: calling `recv` on an empty channel will spin and yield to the OS many times before giving up and parking. While it *might* be good for highly contented benchmark, it adds latency and disrupt the scheduler on each operation. 
...
When no waker is registered, both primitives require a single atomic load, with their waking path outlined in a cold function. Each crate would deserve a dedicated section on its optimizations.

`chenal` code (as well as `aiq`'s and `spmc-waker`'s code) is also carefully designed around hot path inlining. Each operation is compiled to a few dozen assembly instructions, while the outlined cold path has its generic parameters erased to be reused by different generic instances.

The algorithms are optimized to minimize contention and depend on the CPU architecture. On x86_64 where `SeqCst` stores are expensive, they are downgraded to `Release` by adding an unbounded backoff loop on receiver side. Contrary to the classical channel algorithm by Dmitry Vyukov (used in `crossbeam_channel` or `tachyonix`), channel slots are not written by receiver, reducing contention on the slots' cache lines. On aarch64, the channel's tail is also never accessed by the receiver, putting no contention on the senders.
